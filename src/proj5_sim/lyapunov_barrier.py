...
class TurtlebotBarrierDeadlock(TurtlebotBarrier):
    ...
    def eval(self, u, t):
        ...
        #evaluate the CBF
        h = (qo[0, 0] - qe[0, 0])**2 + (qo[1, 0] - qe[1, 0])**2 - self.rt**2 - 0.1

        #evaluate the derivative of the CBF
        hDot = 2 * (qo[0, 0] - qe[0, 0]) * (qoDot[0, 0] - qeDot[0, 0]) -  2 * (qo[1, 0] - qe[1, 0]) * (qoDot[1, 0] - qeDot[1, 0])
...
class TurtlebotBarrierVision(TurtlebotBarrierDeadlock):
    ...
    def eval(self, u, t):
        ...
        dist = np.linalg.norm(qe - ptcloudSpatial, axis=0)
        colMin = np.argmin(dist) #find the closest column -- this may take more than one line
        qo = ptcloudSpatial[:, colMin] #define the obstacle point from the closest column
        qo = np.expand_dims(qo, 1)

        #assume the obstacle velocities and accelerations are negligible
        qoDot, zo = np.zeros((3,1)), np.zeros((3,1))

        #evaluate the CBF
        h = (qo[0, 0] - qe[0, 0])**2 + (qo[1, 0] - qe[1, 0])**2 - self.rt**2 - 1
